...
import { Award, Zap, UserCircle2, ChevronRight, ShieldCheck, Code2, LayoutDashboard, CheckCircle2, Clock, Sparkles, ExternalLink, Swords, Check, Flame, Trophy, Star, Crown, Target, Shield } from 'lucide-react';
...
export default function Dashboard() {
    ...
    const fetchData = async () => {
        try {
            ...
            const [profRes, leadRes, chalRes] = await Promise.all([
                ...
                fetch(API_BASE_URL + '/api/leaderboard/overall', { headers: { 'Authorization': `Bearer ${token}` } }),
                fetch(API_BASE_URL + '/api/challenge/received', { headers: { 'Authorization': `Bearer ${token}` } })
            ]);
            ...
            // Fetch Daily Task
            try {
                const response = await fetch(API_BASE_URL + '/api/task/today', {
            ...
            // Fetch Streak & Badge Info
            try {
                const streakRes = await fetch(API_BASE_URL + '/api/task/streak-info', {
                    headers: { Authorization: `Bearer ${token}` }
                });
                if (streakRes.ok) {
    ...
    useEffect(() => {
        ...
        // 3-second delayed background refresh to pick up silent auto-sync stats
        const timer = setTimeout(() => {
            fetchData();
        }, 3000);

        // 60-second periodic dashboard stats refresh
    ...
    useEffect(() => {
        ...
        const poll = async () => {
            try {
                const res = await fetch(API_BASE_URL + '/api/task/check-completion', {
                    headers: { Authorization: `Bearer ${token}` }
                });
                ...
                if (data.completed) {
                    ...
                    // Build toast message
                    let toastMsg = '🎉 Task Auto-Completed!';
                    if (data.pointsEarned) {
                        toastMsg += ` +${data.pointsEarned} Points!`;
                    }
                    if (data.newBadgesEarned && data.newBadgesEarned.length > 0) {
                        toastMsg += ` 🏆 New Badge!`;
                    }
                    
                    setChallengeToast(toastMsg);
    ...
    const handleManualComplete = async () => {
        ...
        try {
            const res = await fetch(API_BASE_URL + '/api/task/complete', {
                method: 'POST',
                headers: { 
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({ taskId: dailyTask.id })
            });
            ...
            if (res.ok) {
                ...
                // Build toast message
                let toastMsg = `✅ Task Completed! +${data.pointsEarned} Points!`;
                if (data.newBadgesEarned && data.newBadgesEarned.length > 0) {
                    toastMsg += ` 🎉 New Badge Earned!`;
                }
                if (data.freezeTokensEarned > 0) {
                    toastMsg += ` 🛡️ +${data.freezeTokensEarned} Freeze Token!`;
                }
    ...
    const handleSync = async () => {
        try {
            ...
            const token = localStorage.getItem('token');
            if (!token) {
                setChallengeToast('Authentication required. Please login again.');
            ...
            const res = await fetch(API_BASE_URL + '/api/sync/fetch-leetcode', {
                method: 'POST',
                headers: { 'Authorization': `Bearer ${token}` },
            ...
            if (res.ok) {
                ...
                setProfile((prev: any) => {
                    ...
                    return { 
                        ...prev, 
                        leetcodePoints: data.totalPoints || prev.leetcodePoints,
                        totalPoints: data.totalPoints || prev.totalPoints
                    };
                });
                setChallengeToast('Stats synced successfully! 🎉');
                setTimeout(() => setChallengeToast(null), 3000);
                // Refresh other data without affecting loading state
                await refreshDashboardData();
            } else {
                const errorData = await res.json().catch(() => ({}));
                console.error('Sync failed:', errorData);
                setChallengeToast(`Sync failed: ${errorData.error || 'Unknown error'}`);
                setTimeout(() => setChallengeToast(null), 4000);
            }
        } catch (err: any) {
            console.error('Sync error:', err);
            if (err.name === 'AbortError') {
                setChallengeToast('Sync timed out. Please try again.');
            } else {
                setChallengeToast('Network error during sync. Please try again.');
            }
            setTimeout(() => setChallengeToast(null), 4000);
        } finally {
            setSyncing(false);
    ...
    // Separate function to refresh dashboard data without affecting loading state
    ...
    const handleChallengeClick = async (sub: any) => {
        try {
            ...
            const response = await fetch(API_BASE_URL + '/api/challenge/send', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
    ...
    const markChallengeDone = async (id: string) => {
        try {
            const token = localStorage.getItem('token');
            const res = await fetch(`${API_BASE_URL}/api/challenge/${id}/complete`, {
                method: 'PUT',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if (res.ok) {
    ...
    const refetchChallenges = async () => {
        try {
            const token = localStorage.getItem('token');
            const res = await fetch(API_BASE_URL + '/api/challenge/received', {
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if (res.ok) {
                const data = await res.json();
        ...
        <DashboardLayout>
            <motion.div variants={containerVariants} initial="hidden" animate="visible">
                ...
                <motion.div variants={itemVariants} className="flex gap-4 flex-wrap mb-8">
                    <motion.div whileHover={{ y: -5 }} style={{ flex: 1, minWidth: '200px', display: 'flex' }}>
                        <StatCard title="LeetCode Points" value={profile.leetcodePoints || 0} icon={Award} color="251, 146, 60" />
                    </motion.div>
                    <motion.div whileHover={{ y: -5 }} style={{ flex: 1, minWidth: '200px', display: 'flex' }}>
                        <StatCard title="Total Solved" value={profile.stats?.totalSolved || 0} icon={Code2} color="99, 102, 241" />
                    </motion.div>
                </motion.div>

                {/* Task Streak & Badges Widget */}
                {streakInfo && (
                    <motion.div 
                        variants={itemVariants} 
                        className="glass-card hover-lift mb-8"
                        style={{
                    ...
                    >
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
                            <h3 className="text-xl font-black flex items-center gap-2" style={{ margin: 0 }}>
                                ...
                                {streakInfo.streakFreezes > 0 && (
                                    <div style={{
                                        ...
                                        <span style={{ fontSize: '11px', fontWeight: 700, color: '#3b82f6', letterSpacing: '0.05em' }}>
                                            {streakInfo.streakFreezes} FREEZE{streakInfo.streakFreezes > 1 ? 'S' : ''}
                                        </span>
                                    </div>
                                )}
                                <div style={{
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem' }}>
                            ...
                            <div style={{
                                ...
                                <div style={{ fontSize: '11px', fontWeight: 700, color: 'var(--text-muted)', marginTop: '8px', letterSpacing: '0.1em', textTransform: 'uppercase' }}>
                                    Current Streak
                                </div>
                            </div>
                            ...
                            <div style={{
                                ...
                                <div style={{ fontSize: '3rem', fontWeight: 900, color: '#ef4444', lineHeight: 1 }}>
                                    {streakInfo.longestTaskStreak}
                                </div>
                                <div style={{ fontSize: '11px', fontWeight: 700, color: 'var(--text-muted)', marginTop: '8px', letterSpacing: '0.1em', textTransform: 'uppercase' }}>
                                    Record Streak
                                </div>
                            ...
                            {streakInfo.nextMilestone ? (
                                <div style={{
                                    ...
                                    <div style={{ fontSize: '2.5rem', marginBottom: '8px' }}>
                                        {streakInfo.nextMilestone.nextBadge.icon}
                                    </div>
                                    <div style={{ fontSize: '13px', fontWeight: 800, color: 'var(--text-primary)', marginBottom: '4px' }}>
                                        {streakInfo.nextMilestone.nextBadge.name}
                                    </div>
                                    <div style={{ fontSize: '11px', fontWeight: 600, color: 'var(--text-muted)' }}>
                                        {streakInfo.nextMilestone.daysUntil} day{streakInfo.nextMilestone.daysUntil > 1 ? 's' : ''} to go!
                                    </div>
                                </div>
                            ) : (
                                <div style={{
                                    ...
                                    <Trophy size={40} style={{ color: '#f59e0b', marginBottom: '8px' }} />
                        ...
                        {streakInfo.badges && streakInfo.badges.length > 0 && (
                            <div style={{ marginTop: '1.5rem' }}>
                                <div style={{ fontSize: '10px', fontWeight: 700, color: 'var(--text-muted)', marginBottom: '12px', letterSpacing: '0.15em', textTransform: 'uppercase' }}>
                                    Earned Badges ({streakInfo.badges.length})
                                </div>
                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
                                    {streakInfo.badges.map((badge: any) => (
                                        <div
                                            key={badge.key}
                                            style={{
                                                display: 'flex',
                ...
                <div className="flex gap-6 flex-wrap">
                    ...
                    <div style={{ flex: '2 1 600px' }} className="flex-col gap-6">
                                ...
                                dailyTask.completed ? (
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                                        ...
                                        <button
                                            ...
                                            onMouseEnter={(e) => {
                                                e.currentTarget.style.background = 'linear-gradient(90deg, rgba(16,185,129,0.25), rgba(5,150,105,0.2))';
                                                e.currentTarget.style.transform = 'translateY(-1px)';
                                            }}
                                            onMouseLeave={(e) => {
                                                e.currentTarget.style.background = 'linear-gradient(90deg, rgba(16,185,129,0.15), rgba(5,150,105,0.1))';
                                                e.currentTarget.style.transform = 'translateY(0)';
                                            }}
                                        >
                                            <Check size={16} />
                        ...
                        <motion.div variants={itemVariants} className="glass-card hover-lift mt-6" style={{ position: 'relative' }}>
                            ...
                            <div 
                                className="flex flex-col gap-3 pr-2 styled-scrollbar"
                                style={{
                                    maxHeight: '400px',
                                    overflowY: 'auto',
                            ...
                            >
                                {(() => {
                                    // Parse recentSubmission if it's a JSON string
                                    let submissions: any[] = [];
                                    if (profile.stats?.recentSubmission) {
                                        if (typeof profile.stats.recentSubmission === 'string') {
                                            try {
                                                submissions = JSON.parse(profile.stats.recentSubmission);
                                            } catch (e) {
                                                console.error('Failed to parse recentSubmission:', e);
                                                submissions = [];
                                            }
                                        } else if (Array.isArray(profile.stats.recentSubmission)) {
                                            submissions = profile.stats.recentSubmission;
                                        }
                                    }
                                    
                                    return submissions.length > 0 ? (
                                        submissions.slice(0, 50).map((sub: any, idx: number) => (
                                            <div key={idx} className="flex items-center justify-between p-3 rounded-xl bg-elevated/50 border border-border-color/30 hover:bg-elevated transition-all duration-200" style={{ gap: '1rem' }}>
                                                <div className="flex-col" style={{ flex: 1, minWidth: 0 }}>
                                                    <div className="font-bold text-sm truncate" title={sub.title}>{sub.title}</div>
                                                ...
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexShrink: 0 }}>
                                                    ...
                                                    <button
                                                        ...
                                                        style={{ fontSize: 10, fontWeight: 700, padding: '4px 10px', borderRadius: 8, background: 'rgba(99,102,241,0.1)', color: 'var(--accent-primary)', border: '1px solid rgba(99,102,241,0.2)', cursor: 'pointer', whiteSpace: 'nowrap', transition: 'all 0.2s' }}
                                                        onMouseEnter={e => {
