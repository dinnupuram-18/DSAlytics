...
import { clearCache, clearCacheByPrefix } from './cacheService';
...
export const syncUserData = async (userId: string) => {
    console.log('[Scraper] Starting sync for user:', userId);
    const user = await prisma.user.findUnique({ where: { id: userId } });
    if (!user) throw new Error('User not found');

    console.log('[Scraper] User found:', user.name, 'LeetCode URL:', user.leetcodeUrl);
    ...
    const platformFetchResults = await Promise.all(
        platforms.map(async (p) => {
            if (!p.url) return { name: p.name, stats: null, isConfigured: false };
            const username = extractUsername(p.url, p.name);
            if (!username) return { name: p.name, stats: null, isConfigured: false };
            const stats = await fetchPlatformStats(username, p.name);
            return { name: p.name, stats, isConfigured: true };
        })
    );
    ...
    let leetcodeRecentSubmissions: any[] = currentStats?.recentSubmission ? JSON.parse(currentStats.recentSubmission as string) : [];

    for (const { name, stats, isConfigured } of platformFetchResults) {
        if (!isConfigured) {
            // Platform has no URL configured. Set points to 0.
            individualPoints[name] = 0;
            platformStats[name] = { connected: false };
            continue;
        }

        if (!stats) {
            anyScrapeFailed = true;
            // Scrape failed. Use EXISTING points from DB for this platform.
            ...
            stats: {
                upsert: {
                    create: {
                        ...
                        recentSubmission: JSON.stringify(leetcodeRecentSubmissions)
                    },
                    update: {
                        totalSolved: aggregatedStats.totalSolved,
                        easySolved: aggregatedStats.easy,
    ...
    // Invalidate caches upon successful stats update
    clearCache(`profile:${userId}`);
    clearCacheByPrefix('leaderboard:overall');
