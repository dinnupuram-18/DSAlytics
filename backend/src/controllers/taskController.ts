...
import { checkTaskEligibility, generateDailyTask, completeDailyTask, checkAutoCompletion, BADGE_DEFINITIONS, getNextMilestone } from '../services/taskService';
...
/**
 * Get user's streak and badge information
 */
export const getUserStreakInfo = async (req: Request, res: Response) => {
    try {
        const userId = (req as any).user?.id;
        if (!userId) return res.status(401).json({ error: "Unauthorized" });
        ...
        try {
            earnedBadges = user.badges ? JSON.parse(user.badges) : [];
        } catch (e) {
            earnedBadges = [];
        }

        // Get badge details
        const badgeDetails = earnedBadges.map(badgeKey => ({
            key: badgeKey,
            ...BADGE_DEFINITIONS[badgeKey as keyof typeof BADGE_DEFINITIONS]
        })).filter(b => b.name); // Filter out undefined badges

        // Get next milestone
        const nextMilestone = getNextMilestone(user.taskStreak);

        res.json({
            taskStreak: user.taskStreak,
            longestTaskStreak: user.longestTaskStreak,
