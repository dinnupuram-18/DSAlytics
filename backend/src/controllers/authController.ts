...
export const getProfile = async (req: any, res: Response): Promise<void> => {
    try {
        ...
        // Trigger automatic silent background sync (throttled to once every 2 minutes)
        ...
        if (!alreadySyncedRecently) {
            // Set throttle for 2 minutes (120 seconds)
            setCache(syncCacheKey, true, 120);
            
            // Execute silent sync in the background
            syncUserData(userId).catch(err => {
                console.error(`Automatic background sync failed for user ${userId}:`, err);
            });
        }
...
export const updateProfile = async (req: any, res: Response): Promise<void> => {
    try {
        ...
        // Trigger background sync to pull fresh data from the updated URLs immediately
        syncUserData(userId).catch(err => {
            console.error(`Background sync failed for user ${userId} on profile update:`, err);
        });
