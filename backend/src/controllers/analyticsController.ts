...
export const getPlacementReadiness = async (req: any, res: Response): Promise<void> => {
    try {
        ...
        let scrapedStats: Record<string, number> = {};
        if (user.topicStats) {
            try {
                scrapedStats = typeof user.topicStats === 'string' ? JSON.parse(user.topicStats) : user.topicStats;
            } catch (e) {
                console.error('Error parsing topicStats:', e);
            }
        }
